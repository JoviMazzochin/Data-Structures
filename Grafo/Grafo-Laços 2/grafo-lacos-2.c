...
int verificaLacoPosicao(grafo_t* g, int i){
    if(g->M[i][i] == 1){
        return 1;
    }
    return 0;
...
grafo_t* g = criaGrafo(9);
    ...
    g = removeVertice(g,8); //Figura 4
    removerArestaDirecionada(g,7,0);
    verificaLaco(g);
    ...
    removerArestaDirecionada(g,3,3);//Figura 8
    verificaLaco(g);
